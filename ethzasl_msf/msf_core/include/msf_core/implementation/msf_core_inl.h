...
void MSF_Core<EKFState_T>::ProcessIMU(
  ...
  //std::cout<<"currentState->a_m "<<currentState->a_m<<std::endl;
  //std::cout<<"currentState->w_m "<<currentState->w_m<<std::endl;
  //std::cout<<"currentState->noise_gyr "<<currentState->noise_gyr<<std::endl;
  //std::cout<<"currentState->noise_acc "<<currentState->noise_acc<<std::endl;
  ...
  if (currentState->a_m.norm() > 50)
    ...
    //std::cout<<" last_am "<< last_am<<std::endl;
...
//ROS_INFO("addmeasurement S1");
  ...
  if (measurement->time < stateBuffer_.GetFirst()->time) {
    ...
    //ROS_INFO("addmeasurement S2a");
    return;  // Reject measurements too far in the past.
  }
  //ROS_INFO("addmeasurement S2b");
...
//ROS_INFO("addmeasurement S3");
  ...
  ROS_INFO("!!!Publish stateafterupdate");
}
...
bool MSF_Core<EKFState_T>::ApplyCorrection(shared_ptr<EKFState_T>& delaystate,
  ...
  ROS_INFO(" Call correction function for every state.");
